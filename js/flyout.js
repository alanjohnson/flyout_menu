...
$.fn.addFlyout = function(options) {
  ...
  $.each(options.item_data, function(key, data) {
    if (data !== 0) {
      flyout_container.append('<a href="'+data.url+'" class="flyout_link '+key+' '+options.flyout_theme+'  " alt="'+data.name+'" data-title="'+data.name+'" style="width:'+options.flyout_size+'px;height:'+options.flyout_size+'px"><div class="flyout_tooltip">'+data.name+'</div></a>');
  ...
  function showLinks() {
      mousein = true;
      flyout_container.addClass('hover');
      if (!visible) {
          visible = true;
          flyout_container.find('.flyout_link').each(function(index) {
              ...
              if ( ( center.x - (options.flyout_size/2) + (Math.cos(theta)*(options.flyout_radius / 2) ) ) <= 0 ) {
                $(this).addClass('left');
              }
              $(this).stop().animate({
  ...
  function hideLinks() {
      ...
      window.setTimeout(function() {
          if (visible && !mousein) {
              visible = false;
              flyout_container.removeClass('hover');
              flyout_container.find('.flyout_link').each(function() {
                  speed = Math.floor(Math.random() * 301);
...
$(document).ready(function() {
  ...
  $('.flyout_menu').bind('mouseenter', function(e){
